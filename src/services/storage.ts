...
import {AppAlert,AuthSession,Complaint,Message,Provider,Request,Review,Service,UserProfile,ProviderAccountStatus,ProviderAvailability} from '../types';
const keys={requests:'fixmate.requests',favourites:'fixmate.favourites',messages:'fixmate.messages',alerts:'fixmate.alerts',reviews:'fixmate.reviews',profile:'fixmate.profile',providerStatuses:'fixmate.providerStatuses',availability:'fixmate.availability',session:'fixmate.session',complaints:'fixmate.complaints',providers:'fixmate.providers',services:'fixmate.services'};
...
export const loadAlerts=()=>read<AppAlert[]>(keys.alerts,[]); export const saveAlerts=(v:AppAlert[])=>write(keys.alerts,v);
...
export const loadProviders=()=>read<Provider[]>(keys.providers,[]); export const saveProviders=(v:Provider[])=>write(keys.providers,v);
export const loadServices=()=>read<Service[]>(keys.services,[]); export const saveServices=(v:Service[])=>write(keys.services,v);
